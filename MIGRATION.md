...
| 5 | Migration Leaflet, incrémentale, couche par couche | Phase 3 + Phase 4 | **Le plus élevé** (~85-90 fichiers) | Outils interactifs (règle, minimap, labels) les plus exposés | **En cours** (caméra Leaflet faite, couches GeoJSON pas encore) |
...
**Ce qui reste, explicitement pas encore fait** (la partie qui a motivé les deux décisions ci-dessus, et le plus gros du risque de la phase) :
1. Les couches d'affichage GeoJSON elles-mêmes (biomes/états/provinces/cultures/religions/rivières/routes/burgs/marqueurs) comme vraies couches `L.geoJSON()` — la donnée existe déjà (`getIsolines(..., { polygons: true })` dans `src/utils/pathUtils.ts` retourne déjà exactement les anneaux `[x,y]` nécessaires, zéro nouveau code de géométrie à écrire) et le mécanisme de pane est designé (pane Leaflet standard, enfant du pane interne de Leaflet — de par l'ordre d'ajout au DOM, tout ce qui est dedans peint nécessairement sous `legacySvg`, un compromis assumé, pas une interleaving parfaite). **Tentative faite puis annulée pendant cette passe** pour provinces/cultures/religions/biomes : `provinces-editor.ts`, `cultures-editor.ts`, `religions-editor.ts` et `biomes-editor.ts` requêtent aujourd'hui les tracés individuels par id (`#province5`, `#culture12`, etc., avec parfois un ancêtre `#provs`/`#cults`/`#relig` exigé dans la même chaîne de sélection) pour le surlignage au survol, la recoloration en direct depuis l'éditeur, et le nettoyage à la suppression. Remplacer le rendu par de vraies couches `L.geoJSON()` sans adapter ces quatre éditeurs casserait silencieusement ce feedback en direct — une vraie régression fonctionnelle sur de l'édition active, pas un simple écart visuel cosmétique. Donc : convertir le rendu d'une couche de territoire ET adapter son éditeur associé forment un seul lot de travail, pas deux étapes séparables. `states-editor.ts`/`draw-states.ts` a en plus son propre effet de halo (`<clipPath>` + `<use href="#state{id}">`) sans équivalent Leaflet direct, non revalidé.
2. La délégation de clics (`viewbox-events.ts`) pour `rivers`/`routes`/`burgIcons`/`markers` vers des clics natifs par-feature Leaflet, une fois ces couches converties.
...
7. `services/io/auto-update.ts` : à vérifier une fois `rivers`/`routes` convertis — si leur rendu ne passe plus du tout par `<path id="river{i}">` dans `#viewbox`, ses 4 blocs de migration legacy (qui lisent ces éléments) auront besoin d'un rendu de secours isolé, hors-écran.
