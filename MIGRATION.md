...
| 5 | Migration Leaflet, incrémentale, couche par couche | Phase 3 + Phase 4 | **Le plus élevé** (~85-90 fichiers) | Outils interactifs (règle, minimap, labels) les plus exposés | **En cours** (caméra + `biomes` + `religions` + `cultures` + `provinces` faits ; states/rivers/routes/burgs/marqueurs pas encore) |
...
1. `states` — `draw-states.ts` a son propre effet de halo (`<clipPath>` + `<use href="#state{id}">`) sans équivalent Leaflet direct, non revalidé. Auditer `states-editor.ts` en entier avant de commencer (le plus gros fichier de ce type converti jusqu'ici) — s'attendre à trouver, comme pour `provinces`, au moins une lecture de géométrie DOM (`d`/`getBBox`) au-delà du simple survol/recoloration/suppression déjà vus trois fois.
