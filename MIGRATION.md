...
1. Les couches d'affichage GeoJSON elles-mêmes (biomes/états/provinces/cultures/religions/rivières/routes/burgs/marqueurs) comme vraies couches `L.geoJSON()` dans des panes Leaflet natifs, sandwichées autour du pane `legacySvg` — la donnée existe déjà (`getIsolines(..., { polygons: true })` dans `src/utils/pathUtils.ts` retourne déjà exactement les anneaux `[x,y]` nécessaires, zéro nouveau code de géométrie à écrire).
