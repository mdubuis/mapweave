...
function openInArmoria(): void {
  ...
  const url = `https://azgaar.github.io/Armoria/?coa=${json}&from=Mapweave`;
  openURL(url);
}
...
async function downloadGallery(): Promise<void> {
  ...
  const mapweave = `<a href="https://github.com/mdubuis/mapweave" target="_blank">Mapweave</a>`;
  ...
  const html = /* html */ `<!DOCTYPE html>
    <html>
      ...
      <body>
        ...
        <address>Generated by ${mapweave}. The tool is free, but images may be copyrighted, see ${license}</address>
      </body>
    </html>`;
